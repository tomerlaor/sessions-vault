...
type Step = 'loading' | 'no-config' | 'section' | 'instruments' | 'chords' | 'bars' | 'timesig' | 'style' | 'preview' | 'generating' | 'error'
...
export default function TabAgentModal({ project, onInsert, onClose }: Props) {
  ...
  const [wizard, setWizard]       = useState<WizardState>({
    sectionName: '', instruments: ['guitar', 'bass', 'drums'],
    chords: '', bars: 8, timeSignature: project.timeSignature ?? '4/4',
    styleNote: '',
  })
  const [preview, setPreview]     = useState<TabPart[]>([])
  const [errorMsg, setErrorMsg]   = useState('')
  const [lyricsHint, setLyricsHint] = useState<{ name: string; chords: string }[]>([])
  ...
  useEffect(() => {
    getAIConfig().then(cfg => {
      if (!cfg?.apiKey && (cfg?.provider === 'openai' || cfg?.provider === 'anthropic')) {
        setStep('no-config')
        return
      }
      if (!cfg) { setStep('no-config'); return }
      setAiCfg(cfg)
      ...
      if (sections.length > 0) {
        addAgent(
          ...
          sections.map(s => `• ${s.name}${s.chords ? ` — ${s.chords}` : ''}`).join('\n') +
          `\n\nWhich section would you like to generate a tab structure for?`
        )
      } else {
        addAgent("Let's build a tab structure. What's the name of this section? (e.g. Verse, Chorus, Bridge)")
      }
      setStep('section')
    })
  }, [project.lyrics])
  ...
  const goToInstruments = (sectionName: string, chords: string) => {
    ...
    setWizard(next)
    addUser(sectionName)
    addAgent(`Which instruments should I generate for this section? Toggle on or off below.`)
    setStep('instruments')
  }
  ...
  const goToChords = () => {
    ...
    if (wizard.chords) {
      addAgent(`I'll use the chords from your lyrics: ${wizard.chords}\n\nFeel free to edit them below, or confirm to continue.`)
    } else {
  ...
  const goToTimeSig = (bars: number) => {
    ...
    if (project.timeSignature) {
      addAgent(`Your project is set to ${project.timeSignature}. Confirm or change the time signature below.`)
    } else {
  ...
  const goToStyle = (timeSig: string) => {
    ...
    addUser(timeSig)
    addAgent(`Any playing feel or style notes? (optional — e.g. "palm muted", "arpeggiated", "funk groove")\nLeave blank to skip.`)
    setStep('style')
  }
  ...
  const goToPreview = (styleNote: string) => {
    ...
    const instList = finalWizard.instruments.map(i => `${finalWizard.sectionName} ${i.charAt(0).toUpperCase() + i.slice(1)}`).join(', ')
    addAgent(
      `Ready to generate!\n\n` +
  ...
  return (
    <div className="agent-overlay" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="agent-modal">

        {/* Header */}
        <div className="agent-header">
          <span className="agent-title">✦ Tab Structure Generator</span>
          <button onClick={onClose} style={{ color: 'var(--text-3)', padding: '2px 6px' }}>✕</button>
        </div>
          ...
          {step === 'no-config' && (
            <div className="agent-bubble agent">
              ...
              <div className="agent-bubble-text">
                No AI provider configured yet. Open <b>Settings → AI Assistant</b> to add your API key or local runner URL, then come back.
              </div>
            </div>
          )}
        ...
        <div className="agent-input-area">
          ...
          {step === 'instruments' && (
            <InstrumentsStep
              ...
              onChange={insts => setWizard(w => ({ ...w, instruments: insts }))}
          ...
          {step === 'style' && (
            <StyleStep onConfirm={goToPreview} />
          )}

          {step === 'preview' && preview.length === 0 && (
            <button className="tb-btn primary" style={{ width: '100%', justifyContent: 'center' }}
          ...
          {step === 'preview' && preview.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <button className="tb-btn primary" style={{ width: '100%', justifyContent: 'center', marginTop: 4 }}
          ...
          {step === 'error' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ fontSize: 11, color: 'var(--red)', background: 'rgba(255,90,90,0.08)',
                border: '1px solid rgba(255,90,90,0.3)', borderRadius: 6, padding: '8px 10px' }}>
                {errorMsg}
              </div>
              <button className="tb-btn" onClick={() => setStep('preview')}>← Try again</button>
            </div>
          )}
        </div>
      </div>
...
function SectionStep({ hints, onConfirm }: {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      {hints.map(h => (
        <button key={h.name} className="agent-choice-btn" onClick={() => onConfirm(h.name, h.chords)}>
          <span style={{ fontWeight: 600 }}>{h.name}</span>
          {h.chords && <span style={{ color: 'var(--text-3)', fontSize: 11 }}>{h.chords}</span>}
        </button>
      ))}
      <div style={{ display: 'flex', gap: 6 }}>
        ...
        />
        <button className="tb-btn primary" disabled={!custom.trim()}
...
function InstrumentsStep({ selected, onChange, onConfirm }: {
  selected: Instrument[]
  onChange: (i: Instrument[]) => void
  onConfirm: () => void
}) {
  const toggle = (inst: Instrument) =>
    onChange(selected.includes(inst) ? selected.filter(i => i !== inst) : [...selected, inst])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <button className="tb-btn primary" disabled={selected.length === 0}
        style={{ alignSelf: 'flex-start' }} onClick={onConfirm}>
        Confirm →
      </button>
    </div>
  )
...
function ChordsStep({ initial, onConfirm }: { initial: string; onConfirm: (c: string) => void }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 6 }}>
      ...
      />
      <button className="tb-btn primary" onClick={() => onConfirm(val.trim())}>→</button>
    </div>
  )
}

function BarsStep({ initial, onConfirm }: { initial: number; onConfirm: (n: number) => void }) {
  const [val, setVal] = useState(initial)
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={{ display: 'flex', gap: 6 }}>
        ...
        <input
          ...
          style={{
            width: 56, background: 'var(--bg-2)', border: '1px solid var(--line)',
            borderRadius: 4, padding: '4px 8px', fontSize: 12, color: 'var(--text-0)',
            textAlign: 'center',
          }}
        />
      </div>
      <button className="tb-btn primary" style={{ alignSelf: 'flex-start' }}
        onClick={() => onConfirm(val)}>Confirm →</button>
    </div>
  )
}

function TimeSigStep({ initial, onConfirm }: { initial: string; onConfirm: (s: string) => void }) {
  const [val, setVal] = useState(initial)
...
function StyleStep({ onConfirm }: { onConfirm: (s: string) => void }) {
  ...
  const SUGGESTIONS = ['palm muted', 'arpeggiated', 'strummed', 'funk groove', 'fingerpicked']
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        {SUGGESTIONS.map(s => (
          <button key={s} className="tb-btn"
            style={val === s ? { background: 'var(--bg-3)', color: 'var(--text-0)' } : {}}
