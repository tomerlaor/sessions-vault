...
const CHORD_RE =
  /\[([A-G][b#]?(?:sus[24]?(?:maj|min|m|M)?[0-9]*|(?:maj|min|dim|aug|dom|alt|add|m|M)?[0-9]*)(?:[b#][0-9]+)*(?:\/[A-G][b#]?)?)\]/g;
...
function ChordRow({ line, rtl }: { line: string; rtl: boolean }) {
  ...
  if (!hasChords) {
    return (
      <p style={{ margin: "0 0 2px", color: "var(--text-0)" }}>{line || " "}</p>
    );
  }
  ...
  return (
    <div className="lyrics-content" style={{ direction: rtl ? "rtl" : "ltr" }}>
      {lines.map((line, i) => {
        ...
        if (
          /^\[[^\]]+\]$/.test(trimmed) &&
          !/^\[([A-G][b#]?(?:sus[24]?(?:maj|min|m|M)?[0-9]*|(?:maj|min|dim|aug|dom|alt|add|m|M)?[0-9]*)(?:[b#][0-9]+)*(?:\/[A-G][b#]?)?)\]$/.test(
            trimmed,
          )
        ) {
          return (
...
export default function LyricsTab({ project: p, onUpdate }: Props) {
  ...
  const handleOverlayInsert = (
    ...
    setDraft((prev) => {
      ...
      if (mode === "alternative") {
        return (
          prev.slice(0, selectionStart) + suggestion + prev.slice(selectionEnd)
        );
      }
    ...
    <div className="lyrics-editor">
      <div className="lyrics-toolbar">
        ...
        <button
          ...
          onClick={editing ? handleDone : startEdit}
        >
          {editing ? "Done" : "✎ Edit"}
        </button>
      </div>
      ...
      {editing ? (
        <div style={{ display: "flex", flexDirection: "column", flex: 1 }}>
          ...
          >
            ...
            &nbsp;&nbsp;·&nbsp;&nbsp;
            <b style={{ color: "var(--text-2)" }}>[Chorus]</b> section header
            (whole line) &nbsp;&nbsp;·&nbsp;&nbsp;
