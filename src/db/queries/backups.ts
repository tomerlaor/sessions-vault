...
export async function getBackups(projectId: string): Promise<BackupRecord[]> {
  ...
  return rows.map(r => ({ ...r, provider: r.provider as BackupRecord['provider'], status: r.status as BackupRecord['status'] }))
}

export async function insertBackup(record: Omit<BackupRecord, 'id'>): Promise<void> {
  const db = await getDb()
...
export async function getLocalConfig(): Promise<LocalConfig | null> {
  const db = await getDb()
  const rows = await db.select().from(backupConfigs).where(eq(backupConfigs.id, 'local'))
...
export async function setLocalConfig(config: LocalConfig): Promise<void> {
  ...
  await db
    ...
    .onConflictDoUpdate({ target: backupConfigs.id, set: { configJson: JSON.stringify(config) } })
}

export async function getGDriveConfig(): Promise<GDriveConfig | null> {
  const db = await getDb()
  const rows = await db.select().from(backupConfigs).where(eq(backupConfigs.id, 'gdrive'))
  if (!rows[0]) return null
  const safe = JSON.parse(rows[0].configJson) as Pick<GDriveConfig, 'clientId' | 'tokenExpiry'>
  ...
  return { ...safe, clientSecret: clientSecret ?? '', accessToken: accessToken ?? '', refreshToken: refreshToken ?? '' }
}
...
export async function setGDriveConfig(config: GDriveConfig): Promise<void> {
  ...
  if (config.clientSecret) await storeSecret(K_GDRIVE_CLIENT_SECRET, config.clientSecret)
  else                      await deleteSecret(K_GDRIVE_CLIENT_SECRET)
  if (config.accessToken)   await storeSecret(K_GDRIVE_ACCESS_TOKEN,  config.accessToken)
  else                      await deleteSecret(K_GDRIVE_ACCESS_TOKEN)
  if (config.refreshToken)  await storeSecret(K_GDRIVE_REFRESH_TOKEN, config.refreshToken)
...
export async function getGDriveFolderConfig(): Promise<LocalConfig | null> {
  const db = await getDb()
  const rows = await db.select().from(backupConfigs).where(eq(backupConfigs.id, 'gdrive-folder'))
  if (!rows[0]) return null
  return JSON.parse(rows[0].configJson) as LocalConfig
}

export async function setGDriveFolderConfig(config: LocalConfig): Promise<void> {
  const db = await getDb()
  await db
    .insert(backupConfigs)
