...
export async function renameProject(id: string, newTitle: string, newFilePath: string): Promise<void> {
  const db = await getDb()
  await db.update(projects).set({ title: newTitle, filePath: newFilePath }).where(eq(projects.id, id))
}

const EDITABLE_FIELDS = new Set(['status', 'notes', 'key', 'lyrics', 'tabs', 'todos', 'rating'])
